...
func TestGenerateFailsWithShortCredentials(t *testing.T) {
	cg := HMACStrategy{Config: &goauth2.Config{GlobalSecret: []byte("foo")}}
...
func TestValidateSignatureRejects(t *testing.T) {
	var err error
	cg := HMACStrategy{
		Config: &goauth2.Config{GlobalSecret: []byte("1234567890123456789012345678901234567890")},
	}
...
func TestValidateWithRotatedKey(t *testing.T) {
	old := HMACStrategy{Config: &goauth2.Config{GlobalSecret: []byte("1234567890123456789012345678901234567890")}}
	now := HMACStrategy{Config: &goauth2.Config{
	...
	require.EqualError(t, now.Validate(context.Background(), "thisisatoken.withaninvalidsignature"), goauth2.ErrTokenSignatureMismatch.Error())
...
func TestValidateWithRotatedKeyInvalid(t *testing.T) {
	...
	require.EqualError(t, (&HMACStrategy{Config: &goauth2.Config{}}).Validate(context.Background(), token), "a secret for signing HMAC-SHA512/256 is expected to be defined, but none were")
}

func TestCustomHMAC(t *testing.T) {
	def := HMACStrategy{Config: &goauth2.Config{
		GlobalSecret: []byte("1234567890123456789012345678901234567890")},
	}
	sha512 := HMACStrategy{Config: &goauth2.Config{
		GlobalSecret: []byte("1234567890123456789012345678901234567890"),
	...
	require.NoError(t, err)
	require.EqualError(t, sha512.Validate(context.Background(), token), goauth2.ErrTokenSignatureMismatch.Error())

	token512, _, err := sha512.Generate(context.Background())
	require.NoError(t, err)
	require.NoError(t, sha512.Validate(context.Background(), token512))
	require.EqualError(t, def.Validate(context.Background(), token512), goauth2.ErrTokenSignatureMismatch.Error())
}
