...
func TestJWTStrategy_GenerateIDToken(t *testing.T) {
	...
	for k, c := range []struct {
		...
		{
			setup: func() {
				req = goauth2.NewAccessRequest(&DefaultSession{
		...
		},
		{
			...
			setup: func() {
				...
				token, _ := j.GenerateIDToken(context.TODO(), time.Duration(0), goauth2.NewAccessRequest(&DefaultSession{
					Claims: &jwt.IDTokenClaims{
						Subject: "peter",
					},
					Headers: &jwt.Headers{},
