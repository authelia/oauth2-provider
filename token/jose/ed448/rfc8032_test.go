...
// The vectors below are the RFC 8032 Section 7.4 known-answer tests, carried over with the vendored implementation
// so the copy is pinned against the specification rather than against itself.
...
func (v vector) testPublicKey(t *testing.T) {
	...
	if !bytes.Equal(got, want) {
		assert.Equal(t, want, got, "%s", v.name)
	}
}
