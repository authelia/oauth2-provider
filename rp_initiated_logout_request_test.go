...
func TestNewRPInitiatedLogoutRequest_ZeroValues(t *testing.T) {
	...
	assert.Empty(t, request.GetSubject(), "must be empty rather than panic when there is no hint")
	assert.Empty(t, request.GetSessionID(), "must be empty rather than panic when there is no hint")
