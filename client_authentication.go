...
type ClientAuthenticationStrategy interface {
	AuthenticateClient(ctx context.Context, r *http.Request, form url.Values) (client Client, err error)
}
...
func (s *DefaultClientAuthenticationStrategy) authenticate(ctx context.Context, id, secret, assertion, assertionType string, hasBasic, hasPost, hasAssertion bool) (client Client, method string, err error) {
	...
	case 1:
		break
	default:
		if xclient, ok := client.(ClientAuthenticationPolicyClient); !ok || !xclient.GetAllowMultipleAuthenticationMethods(ctx) {
			// TODO: handle error multiple auth types.
		}
	}

	// TODO: handle defined auth method first.

	// TODO: handle in the following order: client_secret_basic, client_secret_post, client_assertion.
...
func (s *DefaultClientAuthenticationStrategy) doAuthenticateAssertionParseAssertionJWTBearer(ctx context.Context, id, assertion string) (client OpenIDConnectClient, method, kid, alg string, token *xjwt.Token, claims *xjwt.RegisteredClaims, err error) {
	...
	if token, err = parser.ParseWithClaims(assertion, claims, func(token *xjwt.Token) (key any, err error) {
		...
		switch {
		case errors.Is(err, &RFC6749Error{}):
	...
	/*
			...
			claims := token.Claims
			var jti string
			if !token.VerifyIssuer(clientID, true) {
				return nil, errorsx.WithStack(ErrInvalidClient.WithHint("Claim 'iss' from 'client_assertion' must match the 'client_id' of the OAuth 2.0 Client."))
...
// ClientAuthenticationLegacyStrategy provides a method signature for authenticating a client request
...
// `oauth2.ClientAuthenticationLegacyStrategy`, if nil it uses `Fosite.DefaultClientAuthenticationStrategy`
...
func clientCredentialsFromRequest(r *http.Request, form url.Values) (id, secret string, err error) {
	...
	switch id, secret, ok, err = getClientCredentialsSecretBasic(r); {
	case err != nil:
...
func getClientCredentialsSecretPost(form url.Values) (id, secret string, ok bool) {
	id, secret = form.Get(consts.FormParameterClientID), form.Get(consts.FormParameterClientSecret)
