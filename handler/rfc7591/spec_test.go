...
//
// TestClientRegistrationHandlerRunsValidators and TestClientRegistrationHandlerEnforcesScopeCeiling
// (handler_registration_test.go) already prove the ErrorField each rejection carries; what neither asserts is the
// HTTP status code the spec actually names, which is the substance of "response shape" here.
...
// =============================================================================
// RFC 7592 §2.1 - Client Read Request
// =============================================================================
...
// =============================================================================
// RFC 7592 §3 - Client Configuration Endpoint Authentication
// =============================================================================
...
func TestSpec_RFC7592_3_InvalidTokenReturns401(t *testing.T) {
	...
	t.Run("OrdinaryAccessToken", func(t *testing.T) {
		...
		// An ordinary session (not implementing Session, i.e. not a client registration token at all) hydrates to a
		// type that fails the Session type assertion inside AuthenticateClientRegistrationRequest, which is what
		// makes a foreign token fail closed - see session.go's DefaultSession doc comment.
	...
	t.Run("CreationTokenAtConfigurationEndpoint", func(t *testing.T) {
		...
		// A creation token presented where a management token is required. This subtest only claims the request is
		// rejected with a 401, which is all RFC 7592 Section 3 requires; it does not isolate the Kind check, because
		// a creation token is audienced to the bare registration endpoint and so the audience check rejects it first.
		// TestAuthRejectsCreateKindSessionAtConfigurationEndpoint (strategy_auth_test.go) is the test that isolates
		// the Kind check, by hand-building a KindCreate session audienced and subjected to the target client so that
...
// §2: "token_endpoint_auth_method [...] OPTIONAL. [...] If unspecified or omitted, the default is
// 'client_secret_basic'".
func TestSpec_OIDCDCR_2_DefaultTokenEndpointAuthMethod(t *testing.T) {
	...
	// GetTokenEndpointAuthMethod is what client_authentication.go actually consults when deciding how a client must
	// authenticate at the token endpoint (see client_authentication.go:279), so this is the live default, not merely
	// a struct field left at its zero value.
...
func TestSpec_OIDCDCR_2_DefaultApplicationTypeIsWeb(t *testing.T) {
	...
	metadata := &oauth2.ClientRegistrationMetadata{
		// ApplicationType is deliberately left empty. A 'native' client's loopback redirect below is disallowed for
		// a 'web' client, so if this were rejected, the default was not applying the 'web' scheme rule.
