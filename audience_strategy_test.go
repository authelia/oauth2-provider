...
// TestDefaultResourceMatchingStrategy validates the RFC 8707 resource indicator matching
// strategy. Needle and haystack values are URL-parsed; a needle matches if scheme + host
// agree and the needle's path is the haystack's path or a sub-path of it.
func TestDefaultResourceMatchingStrategy(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			actual := DefaultResourceMatchingStrategy(tc.haystack, tc.needle)
...
// TestIsMatchingResourceIndicator covers the URL-subpath comparison primitive directly. It
// exercises the scheme/host equality, the path-equal-or-subpath rule, trailing-slash
// normalization, and edge cases around empty paths, root paths, query strings, and fragments.
