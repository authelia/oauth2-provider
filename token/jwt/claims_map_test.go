...
func TestMapClaims_Valid(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			have: MapClaims{consts.ClaimIssuedAt: 1005, consts.ClaimNotBefore: 1010},
		...
		{
			name: "ShouldFailNotBeforeBeyondClockSkew",
			have: MapClaims{consts.ClaimNotBefore: 1011},
			opts: []ClaimValidationOption{ValidateTimeFunc(func() time.Time { return time.Unix(1000, 0) }), ValidateClockSkew(time.Second * 10)},
			errs: []uint32{ValidationErrorNotValidYet},
			err:  "Token is not valid yet",
		},
		{
			name: "ShouldNotApplyClockSkewToExpiry",
			have: MapClaims{consts.ClaimExpirationTime: 999},
			opts: []ClaimValidationOption{ValidateTimeFunc(func() time.Time { return time.Unix(1000, 0) }), ValidateClockSkew(time.Second * 10)},
			errs: []uint32{ValidationErrorExpired},
			err:  "Token is expired",
		},
		{
			name: "ShouldTreatNegativeClockSkewAsZero",
			have: MapClaims{consts.ClaimIssuedAt: 1001},
			opts: []ClaimValidationOption{ValidateTimeFunc(func() time.Time { return time.Unix(1000, 0) }), ValidateClockSkew(-time.Second * 10)},
			errs: []uint32{ValidationErrorIssuedAt},
			err:  "Token used before issued",
		},
