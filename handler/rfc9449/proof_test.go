...
func TestParseProofRejectsWrongType(t *testing.T) {
	key := newTestProofKey(t)
	raw := signProof(t, key, "JWT", map[string]any{"jti": "x", "htm": http.MethodPost, "htu": "https://as/token", "iat": 1})
...
func TestParseProofRejectsBadSignature(t *testing.T) {
	key := newTestProofKey(t)
	raw := signProof(t, key, "dpop+jwt", map[string]any{"jti": "x", "htm": http.MethodPost, "htu": "https://as/token", "iat": 1})
...
func TestParseProofRejectsMissingJTI(t *testing.T) {
	key := newTestProofKey(t)
	raw := signProof(t, key, "dpop+jwt", map[string]any{"htm": http.MethodPost, "htu": "https://as/token", "iat": 1})
...
func TestParseProofRejectsJSONSerialization(t *testing.T) {
	key := newTestProofKey(t)
	raw := signProof(t, key, "dpop+jwt", map[string]any{
		"jti": "json-1", "htm": http.MethodPost, "htu": "https://as.example.com/token", "iat": 1,
	})
