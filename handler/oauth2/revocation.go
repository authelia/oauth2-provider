...
func (r *TokenRevocationHandler) RevokeToken(ctx context.Context, token string, tokenType goauth2.TokenType, client goauth2.Client) error {
	discoveryFuncs := []func() (request goauth2.Requester, err error){
		func() (request goauth2.Requester, err error) {
			// Refresh token
	...
	if ar.GetClient().GetID() != client.GetID() {
		return errorsx.WithStack(goauth2.ErrUnauthorizedClient)
	}
...
func storeErrorsToRevocationError(err1, err2 error) error {
	// both errors are goauth2.ErrNotFound and goauth2.ErrInactiveToken or nil <=> the token is revoked
	if (errors.Is(err1, goauth2.ErrNotFound) || errors.Is(err1, goauth2.ErrInactiveToken) || err1 == nil) &&
		(errors.Is(err2, goauth2.ErrNotFound) || errors.Is(err2, goauth2.ErrInactiveToken) || err2 == nil) {
		return nil
	}

	// there was an unexpected error => the token may still exist and the client should retry later
	return errorsx.WithStack(goauth2.ErrTemporarilyUnavailable)
}
